...
export default function Job() {
  ...
  const fetchAcceptedRequests = async () => {
    ...
    try {
      const response = await fetch('/api/request?status=accepted');
      if (response.ok) {
        ...
        // Get descriptions from accepted jobs
        const acceptedDescriptions = requests
          .map((req: any) => req.description)
          .filter(Boolean);

        if (acceptedDescriptions.length > 0) {
          // Fetch recommendations based on descriptions
          const response = await fetch(
            `/api/job?recommended=true&acceptedDescriptions=${
              encodeURIComponent(acceptedDescriptions.join('|'))
            }&limit=5`
          );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Then fetch recommendations if user is logged in
        if (profileId) {
  ...
  const filteredJobs = sortJobs(jobs.filter((job) => {
    ...
    const aIsRecommended = recommendedJobs.some(rj => rj.id_job === a.id_job);
    const bIsRecommended = recommendedJobs.some(rj => rj.id_job === b.id_job);
  ...
  const sortedJobs = [...jobs].sort((a, b) => {
    ...
    // Then apply existing sort logic
    switch (sortField) {
      case 'time':
        return sortDirection === 'desc' 
          ? new Date(b.time).getTime() - new Date(a.time).getTime()
          ...
          {currentJobs.map((job) => (
            <div key={job.id_job} className="col-md-4 mb-4">
              <div className="card card-custom mx-auto">
                {recommendedJobs.some(rj => rj.id_job === job.id_job) && (
