...
export default function Navigation() {
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      const hasNewRequests = JSON.stringify(data) !== JSON.stringify(pendingRequests);
      if (hasNewRequests) {
        setPendingRequests(data);
        const unreadNotifications = notifications.filter(n => !n.is_read);
        setUnreadCount(unreadNotifications.length + data.length);
      }
    } catch (error) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        const hasNewNotifications = JSON.stringify(data) !== JSON.stringify(notifications);
